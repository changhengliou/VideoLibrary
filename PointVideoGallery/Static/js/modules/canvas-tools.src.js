...
(function(){
	...
	function build() {
		...
		svg.AspectRatio = function(ctx, aspectRatio, width, desiredWidth, height, desiredHeight, minX, minY, refX, refY) {
			...
			// calculate scale
			var scWill = width / desiredWidth;
			var scaleY = height / desiredHeight;
			var scaleMin = Math.min(scWill, scaleY);
			var scaleMax = Math.max(scWill, scaleY);
			...
			if (refX.hasValue() && refY.hasValue()) {				
				...
				if (align.match(/YMid$/) && ((meetOrSlice == 'meet' && scaleMin == scWill) || (meetOrSlice == 'slice' && scaleMax == scWill))) ctx.translate(0, height / 2.0 - desiredHeight / 2.0); 
				if (align.match(/^xMax/) && ((meetOrSlice == 'meet' && scaleMin == scaleY) || (meetOrSlice == 'slice' && scaleMax == scaleY))) ctx.translate(width - desiredWidth, 0); 
				if (align.match(/YMax$/) && ((meetOrSlice == 'meet' && scaleMin == scWill) || (meetOrSlice == 'slice' && scaleMax == scWill))) ctx.translate(0, height - desiredHeight); 
			}
			
			// scale
			if (align == 'none') ctx.scale(scWill, scaleY);
